# SimpleBackup for Valheim by Aloncifer
...
- **Character Name:** Extracted via `Game.instance.m_playerProfile.GetName()`.
- **World Name:** Extracted via `ZNet.instance.GetWorld().m_name`.
...
- Instead of using unstable floating coordinates, the mod locates the native `Save` button game object, Clones it via `GameObject.Instantiate`, re-labels it `"Backup"`, and shifts its mathematical index using `Transform.SetSiblingIndex()`. 
